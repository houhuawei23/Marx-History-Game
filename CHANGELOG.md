...
### 新增
- **AI 生成纪元背景音乐**：使用 music-generation MCP 为三个纪元生成符合历史背景的循环背景音乐
  - 纪元一（原始积累）：18世纪欧洲古典暗色调，弦乐与铜管警告音，70-80 BPM
  - 纪元二（工业垄断）：工业时代戏剧性铜管与打击乐，120-140 BPM
  - 纪元三（AI数字纪元）：未来电子氛围音乐，合成音色与数字故障音，90-100 BPM
  - 音乐文件保存于 `assets/audio/epoch1~3.mp3`
  - 纪元切换时自动通过 `AudioManager.setEpoch()` 进行淡入淡出 crossfade

### 优势
- 沉浸感增强：每个纪元拥有独特的音乐风格，增强历史代入感
- 无缝切换：纪元过渡时音乐平滑淡入淡出，不打断游戏体验
- 自动化生成：使用 AI 自动生成符合历史背景的音乐，无需手动制作
